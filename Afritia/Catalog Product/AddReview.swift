...
class AddReview: UIViewController {
    ...
    func callingHttppApi(){
        ...
        APIServiceManager.shared.callingHttpRequest(params:requstParams, apiname:AfritiaAPI.saveReview, currentView: self){success,responseObject in
            if success == 1{
    ...
    func doFurtherProcessingWithResult(){
        DispatchQueue.main.async {
            ...
            */
            ...
            let addReviewHeading = UILabel(frame: CGRect(x: CGFloat(5), y: CGFloat(mainContainerY), width: SCREEN_WIDTH-10, height: CGFloat(25)))
            addReviewHeading.textColor = UIColor().HexToColor(hexString: "268ED7")
            addReviewHeading.font = UIFont.systemFont(ofSize:18, weight:.semibold)
            addReviewHeading.text = "Rate This Product"
            ...
            if self.ratingFormData.count > 0 {
                ...
                for i in 0..<self.ratingFormData.count {
                    let ratingDict = self.ratingFormData[i]
                    ...
                    starRatingView.tintColor = UIColor.appOrange
            ...
            let submitReviewButton = UIButton(frame: CGRect(x: CGFloat(10), y: CGFloat(mainContainerY), width: CGFloat(self.mainView.frame.size.width - 20), height: CGFloat(50)))
            //submitReviewButton.backgroundColor = UIColor.button
            //submitReviewButton.textColor = UIColor.white
            submitReviewButton.setTitle("submitreview".localized, for: .normal)
            //submitReviewButton.text = GlobalData.sharedInstance.language(key: "submitreview")
            submitReviewButton.applyAfritiaTheme()
            submitReviewButton.addTargetClosure { (btn) in
                self.submitReview()
            }
    ...
    @objc func didChangeValue1(_ sender: HCSStarRatingView) {
        let ratingDict = ratingFormData[sender.tag];
        if sender.value > 0{
        let values:String = (ratingDict["values"][Int(sender.value) - 1]).stringValue
        ratingsDict[ratingDict["id"].stringValue] = values
        }
    }
    ...
    @objc func submitReview() {
        ...
        for i in 0..<ratingFormData.count {
            let ratingDict = ratingFormData[i]
        ...
        if(isValid == 0){
            AlertManager.shared.showErrorSnackBar(msg: errorMessages)
        }
