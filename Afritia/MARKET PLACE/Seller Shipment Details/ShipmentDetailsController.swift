...
class ShipmentDetailsController: UIViewController,UITableViewDelegate, UITableViewDataSource,ShippingTrackingDelegate{
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "shipmentdetails".localized
    ...
    func callingHttppApi(){
        ...
        if whichApiToProcess == "sendtrackinginfo"{
            APIServiceManager.shared.callingHttpRequest(params:requstParams, apiname:"mobikulmphttp/marketplace/sendTrackinginfo", currentView: self){success,responseObject in
                if success == 1{
            ...
            APIServiceManager.shared.callingHttpRequest(params:requstParams, apiname:"mobikulmphttp/marketplace/addtrackinginfo", currentView: self){success,responseObject in
                if success == 1{
                    self.view.isUserInteractionEnabled = true
                    GlobalData.sharedInstance.dismissLoader()
                    var dict = JSON(responseObject as! NSDictionary)
            ...
            APIServiceManager.shared.callingHttpRequest(params:requstParams, apiname:"mobikulmphttp/marketplace/deleteTrackinginfo", currentView: self){success,responseObject in
                if success == 1{
                    self.view.isUserInteractionEnabled = true
                    GlobalData.sharedInstance.dismissLoader()
                    var dict = JSON(responseObject as! NSDictionary)
            ...
            APIServiceManager.shared.callingHttpRequest(params:requstParams, apiname:"mobikulmphttp/marketplace/viewshipment", currentView: self){success,responseObject in
                if success == 1{
                    self.view.isUserInteractionEnabled = true
                    GlobalData.sharedInstance.dismissLoader()
                    var dict = JSON(responseObject as! NSDictionary)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0{
            ...
            cell.sendMailButton.setTitle("sendtrackinginfo".localized, for: .normal)
