...
class ChatMessaging: UIViewController,UITableViewDelegate,UITableViewDataSource,PHFComposeBarViewDelegate {
    ...
    func sendDataToEachSeller(message:String){
        ...
        let fullTokenArray = fullToken.split{$0 == ","}
    ...
    func callingHttppApi(){
        ...
        APIServiceManager.shared.callingHttpRequest(params:requstParams, apiname:"mobikulmphttp/chat/notifyAdmin", currentView: self){success,responseObject in
            if success == 1{
                self.view.isUserInteractionEnabled = true
                GlobalData.sharedInstance.dismissLoader()
                var dict = JSON(responseObject) //as! [String:Any]
